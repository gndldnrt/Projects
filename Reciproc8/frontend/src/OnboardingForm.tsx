...
import { MOCK_PROFILE, SKILLS } from './supabaseClient';
...
export function OnboardingForm({ onComplete }: Props) {
  ...
  const [profile, setProfile] = useState({ fullName: MOCK_PROFILE.full_name, bio: MOCK_PROFILE.bio, zipCode: MOCK_PROFILE.zip_code });
  ...
  function next() {
    if (!profile.fullName.trim() || !/^\d{5}$/.test(profile.zipCode)) { setError('Add your name and a valid 5-digit ZIP code to continue.'); return; }
    setError(''); setStep(2);
  }
  return (
      ...
      <section className="glass relative w-full max-w-4xl rounded-[2rem] p-6 sm:p-10">
        ...
        {step === 1 ? <div className="grid gap-8 md:grid-cols-[1.1fr_.9fr]">
          <div><p className="mb-7 max-w-md text-slate-400">A few details help us surface kindred hobbyists close to you.</p><div className="space-y-5">
            ...
            <label className="block text-sm font-medium text-slate-300">ZIP code<div className="relative mt-2"><MapPin className="absolute left-4 top-3.5 text-slate-500" size={17} /><input className="input-field pl-11" value={profile.zipCode} maxLength={5} onChange={(event) => setProfile({ ...profile, zipCode: event.target.value.replace(/\D/g, '') })} placeholder="10001" /></div></label>
          </div></div>
